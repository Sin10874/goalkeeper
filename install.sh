...
# 从项目自动推断完成条件命令(A 版:不让用户手写 DONE_CMD)
infer_done(){
  local r=$1
  if [ -f "$r/package.json" ] && grep -q '"test"' "$r/package.json" 2>/dev/null; then
    [ -f "$r/pnpm-lock.yaml" ] && { echo "pnpm test"; return; }
    [ -f "$r/yarn.lock" ] && { echo "yarn test"; return; }
    echo "npm test"; return
  fi
  { [ -f "$r/pyproject.toml" ] || [ -f "$r/pytest.ini" ]; } && { echo "pytest -q"; return; }
  [ -f "$r/Cargo.toml" ] && { echo "cargo test"; return; }
  [ -f "$r/go.mod" ] && { echo "go test ./..."; return; }
  if [ -f "$r/Makefile" ]; then
    grep -q '^test:' "$r/Makefile" 2>/dev/null && { echo "make test"; return; }
    grep -q '^check:' "$r/Makefile" 2>/dev/null && { echo "make check"; return; }
  fi
  echo ""
}
...
if [ ! -f "$GKDIR/goal.sh" ]; then
  DC="$(infer_done "$TARGET")"
  { printf 'GOAL="把当前任务做完并通过验证"\n'; printf 'DONE_CMD="%s"\n' "${DC:-false}"; printf 'MAX_TURNS=30\nMAX_SECONDS=0\nDONE_TIMEOUT=120\n'; } > "$GKDIR/goal.sh" || { say "写 goal.sh 失败,中止。"; exit 1; }
  if [ -n "$DC" ]; then say "已自动推断完成条件 DONE_CMD=$DC → .goalkeeper/goal.sh"; else say "未推断出完成条件,请在 .goalkeeper/goal.sh 填 DONE_CMD"; fi
else say "保留已有 .goalkeeper/goal.sh"; fi
...
hr; say "装好了。"
say "  · opencode / pi:在里面用 \`/goal <一句话目标>\` 开始(完成条件已按项目自动推断)。"
say "  · wrapper 档(hermes / openclaw):用 .goalkeeper/goalkeeper-run.sh \"任务\"。"
say "  · 推断不准?改 .goalkeeper/goal.sh 的 DONE_CMD(兜底,非主路径)。"
say "  端到端验证见 TESTING.md;Claude Code 是唯一端到端验证过的(但它有原生 /goal,多数人用原生即可)。"; hr
